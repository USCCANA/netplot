...
```{r nplot-vs-plot, fig.keep='none'}
# Mapping base to grid
ig <- map_base_to_grid(function() plot(UKfaculty))
nw <- map_base_to_grid(function() gplot(as.matrix(as_adj(UKfaculty)), coord = l_ukf))
np <- nplot(UKfaculty, layout = l_ukf)
```

Here is an example with [`ggraph`](https://cran.r-project.org/package=ggraph)

```{r, fig.width=7, fig.height=5, fig.keep='none'}
# This step is done to set a manual layout
# V(UKfaculty)$x <- l_ukf[,1]
# V(UKfaculty)$y <- l_ukf[,2]
# graph_attr(UKfaculty, "node.positions") <- l_ukf

ggraph::ggraph(UKfaculty) +
  ggraph::geom_edge_arc(aes(alpha = ..index..), curvature=.15) +
  ggraph::geom_node_point(aes(colour = Group, size=igraph::degree(UKfaculty))) +
...
```{r, fig.width=7, fig.height=7, fig.cap="Comparision of default `igraph` vs `netplot` default call. `nplot` fills completely the plotting area, and adjusts vertex size, edge width, and edge arrows' size accordingly to the plotting area and plotting device.", fig.keep='last'}
...
gridExtra::grid.arrange(
  nplot(x, layout = l, edge.color = ~ego(mix=0) + alter(mix=1), vertex.size.range = c(.05,.1)),
  nplot(x, layout = l, edge.color = ~ego(mix=.5) + alter(mix=.5), vertex.size.range = c(.05,.1)),
  nplot(x, layout = l, edge.color = ~ego(mix=1) + alter(mix=0), vertex.size.range = c(.05,.1)),
  ncol=3, nrow=1
)
